...
def run(
    ...
    uncertainty_threshold=0,  # Uncertainty threshold for active learning
    ...
    # Activate Dropout layers for uncertainty estimation in active learning
    ...
    def iou(box1, box2):
        ...
        iou_value = inter_area / (
                    box1_area + box2_area - inter_area + 1e-6)  # Add small constant to avoid division by zero
        return iou_value
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        uncertainity_preds = []
        valid_preds = []
        uncertainity_thresold_value = 0.2
        ...
        for key, data in preds_by_object.items():
            ...
            print(f"mean of all confs {mean_confs}")
            class_id = key[1]  # Extract class ID from the key

            if np.all(box_variance >= uncertainity_thresold_value):
